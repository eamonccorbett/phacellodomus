...
default_gen <- read.genepop("genepop_f_M4n4_r95.gen", ncode=3L)
...
s.class(pca_southern$li, as.factor(pop(southern_rufifrons)),xax=1,yax=2, col=southernCol,grid=F,cstar=0,axesell=F,cpoint=2,label=NULL,cellipse=F)

#NJ phylogeny of all individuals
all_dist <- dist(tab(default_gen))
all_tree <- nj(all_dist)
plot(all_tree,type="unrooted")
write.nexus(all_tree, file="r95_tree_all.nex")
